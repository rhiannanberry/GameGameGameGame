...
public class MinigameList 
{
    ...
    public Dictionary<string, List<Minigame>> Authors() {
        Dictionary<string, List<Minigame>> authors = new Dictionary<string, List<Minigame>>();
        foreach(Minigame m in minigames) {
            List<Minigame> old = new List<Minigame>();
            if (!authors.ContainsKey(m.Author)) {
                authors.Add(m.Author, new List<Minigame>());
            }
            authors.TryGetValue(m.Author, out old);
            old.Add(m);
            authors[m.Author] = old;
        }
        return authors;
    }
